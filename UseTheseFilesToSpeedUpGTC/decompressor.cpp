...
{
    //printf("This is PATH 1 !\n");
    struct timeb startTime , endTime , startTime1, endTime1; // Define the time marks
    ...
    uint32_t * revPermByte = nullptr;
    uchar_t * revPermBit = nullptr;
    uint32_t pos;
    
    uint32_t no_haplotypes = smpl.no_samples * pack.s.ploidy;
    perm = new uint32_t[no_haplotypes];
    rev_perm = new uint32_t[no_haplotypes];
    revPermByte = new uint32_t[no_haplotypes];
    revPermBit = new uchar_t[no_haplotypes];
    ...
    else // Belowing is executed when there is -r parameter for query
    {
    	//printf("This is PATH 2 !\n");
        // VCF/BCF processing
        d = (vdict_t*)hdr->dict[BCF_DT_ID];
        k = kh_get(vdict, d, key);
        fmt_id = (k == kh_end(d)? -1 : kh_val(d, k).id);
		...
		long runtimePerm = 0; // runtime of permutations, specificly the function decode_perm_rev
		long iteration = 0; // see how many times the loop runs.
        ...
        while(bcf_itr_next(bcf, itr, record) != -1 && written_records < records_to_process)
        {
            ...
            if(block_id != prev_block_id)
            {
                ...
                pack.getPermArray(block_id, perm);
                //reverse_perm(perm, rev_perm, no_haplotypes);
                reversePerm(perm, revPermByte, revPermBit, no_haplotypes);
            }
            fill_n(decomp_data, pack.s.vec_len*2, 0);
            ...
            ftime(&startTime); // Start time of Permutation
            
            // Permutations
            //decode_perm_rev(no_haplotypes, vec2_start, rev_perm, decomp_data_perm, decomp_data);
            decodePermRev(no_haplotypes, vec2_start, revPermByte, revPermBit, decomp_data_perm, decomp_data);
            
            ftime(&endTime); // End time of Permutation
            runtimePerm = runtimePerm + (endTime.time-startTime.time)*1000 + (endTime.millitm - startTime.millitm); // Accumulate the runtime of Permutation
            
            iteration = iteration + 1; // accumuate the iteration time.
        ...
        fprintf(stderr,"Permutation Runtime: %d ms\n",runtimePerm); // Print the runtime of Permutation
        fprintf(stderr,"Iteration: %d times\n",iteration); // Print the runtime of Permutation
...
// Below are some comments.
	...
	// the length of rev_perm is no_haplotypes, the length of decomp_data_perm and decomp_data is pack.s.vec_len*2, vec2_start = pack.s.vec_len

    //fprintf(stderr,"no_haplotypes: %d \n",no_haplotypes); // Print no_haplotypes
    //fprintf(stderr,"vec2_start: %d \n",vec2_start); // Print vec2_start

	//for (int a = 0;a<10;++a)
	//{
	//	fprintf(stderr,"rev_perm: %d \n",rev_perm[a]); // Print no_haplotypes
	//}
	
	//for (int a = 0;a<vec2_start*2;++a)
	//{
		//if (decomp_data_perm[a])
		//{
			//fprintf(stderr,decomp_data_perm[a]); // Print no_haplotypes
		//}
	//}
	
	//for (int a = 0;a<10;++a)
	//{
	//	fprintf(stderr,"decomp_data: %s \n",decomp_data[a]); // Print no_haplotypes
	//}
	// the length of rev_perm is no_haplotypes, which is 8*pack.s.vec_len, the length of decomp_data_perm and decomp_data is pack.s.vec_len*2, vec2_start = pack.s.vec_len
	//long loop1 = 0;
	//long loop1_2 = 0;
    uint32 x;
    for (x = 0; x + 8 < (uint32) no_haplotypes;)
    {
		...
		//fprintf(stderr,"d_x8: %d \n",d_x8); // Print no_haplotypes
		//fprintf(stderr,"d2_x8: %d \n",d2_x8); // Print no_haplotypes
		
		//loop1_2 += 1;
        //int x_p = 1 << 7;
        for(int x_p = 1 << 7; x_p>0; ++x, x_p >>= 1)
        {
            auto j = rev_perm[x];
            //uint32_t j = rev_perm[x];
	...
	//fprintf(stderr,"loop1: %d \n",loop1); // Print no_haplotypes
	//fprintf(stderr,"loop1_2: %d \n",loop1_2); // Print no_haplotypes
	//fprintf(stderr,"loop2: %d \n",loop2); // Print no_haplotypes
}



void Decompressor::decodePermRev(int no_haplotypes, int vec2_start, uint32_t *revPermByte, uchar_t *revPermBit, uchar_t *decomp_data_perm, uchar_t *decomp_data)
{
    uint32 x;
    for (x = 0; x + 8 < (uint32) no_haplotypes;)
    {
	        ...
	        if (d_x8 & x_p)
	       	{
	        	...
	        	//uchar_t j_p = perm_lut[j % 8];
	        	//int j8 = j / 8;
	            //decomp_data[rev_perm[x] / 8] += perm_lut[rev_perm[x] % 8];
	            decomp_data[revPermByte[x]] += revPermBit[x];
           	}
	        if (d2_x8 & x_p)
	            //decomp_data[vec2_start + rev_perm[x] / 8] += perm_lut[rev_perm[x] % 8];
	            decomp_data[vec2_start + revPermByte[x]] += revPermBit[x];
		    //}
        }
    }
...
void inline Decompressor::reversePerm(uint32_t *perm, uint32_t *revPermByte, uchar_t *revPermBit, int no_haplotypes)
{
    for(int i = 0; i < no_haplotypes; ++i)
               //rev_perm[perm[i]] = i;.
  	{
  		revPermByte[perm[i]] = i/8; // restore which byte the original positions are in
  		revPermBit[perm[i]] = 128 >> (i%8);
  	}
}
